...
class Search extends Component {
  ...
  searchBrands = async () => {
    fetch(url + '/articles?title_contains=' + this.state.searchTerm).then(res => res.json().then(response => { this.setState({ articles: response }) }));
    fetch(url + '/cards?title_contains=' + this.state.searchTerm).then(res => res.json().then(response => { this.setState({ cards: response }) }));
    fetch(url + '/tests?title_contains=' + this.state.searchTerm).then(res => res.json().then(response => { this.setState({ tests: response }) }));
    ...
    return (
      <div className="container">
        <div class="form-inline d-flex justify-content-center md-form form-sm active-pink-2 mt-5">
          <input class="form-control form-control-sm mr-3 w-75 search-input search-box-font purple" type="text" aria-label="Search" onChange={(e) => { this.handleChange(e.target.value) }} value={this.state.searchTerm} id="searchField" placeholder="Search..." />
          <i class="fa fa-search search-icon-modal search-icon-lg" aria-hidden="true" onClick={() => { this.searchBrands() }}></i>
        </div>
        ...
        <div class="row">
          ...
          {cards.length < 1 && articles.length < 1 && tests.length < 1 && searchTerm.trim() != "" && <div className="mx-auto"><h3 className="purple py-5">No result found</h3></div>}
          {this.state.category === "cards" && cards.length === 0 && <div className="mx-auto"><h3 className="purple py-5">No cards found</h3></div>}
            ...
            <div class="col-lg-4 pt-4" key={card._id}>
              <div class="card search-tag-cards">
                <div class="card-header py-4" id="heading-1-1" data-toggle="collapse" role="button"
                  ...
                  <Link class="mb-0"
                    key={card.id} to={`/cards/${card.id}`
                    }>
                    <h6 class="mb-0 search-title-box">{card.title}</h6>
                  </Link>
                  <ul class="article-info">
                    <li>{card.createdAt}</li>
                    <li>{card.author.firstName} {card.author.lastName}</li>
                  </ul>
                </div>
              </div>
            </div>
          ))}
          {this.state.category === "articles" && articles.length === 0 && <div className="mx-auto"><h3 className="purple py-5">No articles found</h3></div>}
          ...
          {this.state.category === "tests" && tests.length === 0 && <div className="mx-auto"><h3 className="purple py-5">No tests found</h3></div>}
          {(this.state.category === 'all' || this.state.category === 'tests') && tests.map((test) => (
            <div class="col-lg-4 pt-4" key={test._id}>
              <div class="card search-tag-tests">
                <div class="card-header py-4" id="heading-1-1" data-toggle="collapse" role="button"
                  data-target="#collapse-1-1" aria-expanded="false" aria-controls="collapse-1-1">
                  <Link class="mb-0 "
                    key={test.id} to={`/test/${test.id}`
                    }>
